...
class CreditSubState extends meta.MusicBeat.MusicBeatSubState
{
    ...
    public function new(name:String, avatar:String, description:String, expression:String, socialMedias:Array<SocialMedia>)
    {
		...
		var posMap = [0 => [800, 200], 1 => [950, 200], 2 => [800, 400], 3 => [950, 400]];
        
        for(i in 0...socialMedias.length)
        {
            var social = new SocialMediaButton(posMap[i][0], posMap[i][1], socialMedias[i].name, socialMedias[i].value);
            add(social);
        }

        FlxTween.tween(bg, {alpha: 0.5}, 0.5, {onComplete: function(_) { canSelect = true; }});
        for(object in [icon, desc, exp]) FlxTween.tween(object, {alpha: 1}, 0.5);
    }
...
class CreditBuddy extends FlxGroup
{
    ...
    override function update(elapsed:Float)
    {
		...
		text.y = FlxMath.lerp(text.y, (targetY * 120) + 320, Math.max(0, Math.min(1, elapsed * 10.2)));
        text.alpha = alpha;
...
{
    public function new(x:Float, y:Float, socialMedia:String, value:String)
    {
        super(x, y);
