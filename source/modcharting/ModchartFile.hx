...
class ModchartFile
{
	...
	{
		data = loadFromJson(state.PlayState.SONG.song.toLowerCase());
	...
	public function loadFromJson(folder:String):ModchartJson // load da shit
	{
		...
		if (rawJson == null)
		{
			...
			#if sys if (FileSystem.exists(filePath))
...
class CustomModifierScript
{
	...
	private function init()
	{
		...
		interp.variables.set('PlayState', state.PlayState);
		interp.variables.set('game', state.PlayState.instance);
		interp.variables.set('Paths', Paths);
		interp.variables.set('Conductor', meta.data.Conductor);
		interp.variables.set('StringTools', StringTools);
		interp.variables.set('Note', gameObjects.userInterface.notes.Note);
