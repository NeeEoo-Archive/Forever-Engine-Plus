...
class ModchartEditorEvent extends FlxSprite
{
	...
	public function new(data:Array<Dynamic>)
	{
		this.data = data;
		super(-300, 0);
		frames = Paths.getSparrowAtlas('noteskins/notes/default/base/NOTE_assets');
...
class ModchartEditorState extends MusicBeatState
{
	...
	public static var modifierList:Array<Class<Modifier>> = [
		DrunkXModifier, DrunkYModifier, DrunkZModifier, TipsyXModifier, TipsyYModifier, TipsyZModifier, ReverseModifier, IncomingAngleModifier,
		RotateModifier, StrumLineRotateModifier, BumpyModifier, XModifier, YModifier, ZModifier, ConfusionModifier, ScaleModifier, ScaleXModifier,
		ScaleYModifier, SpeedModifier, StealthModifier, NoteStealthModifier, InvertModifier, FlipModifier, MiniModifier, ShrinkModifier, BeatXModifier,
		BeatYModifier, BeatZModifier, BounceXModifier, BounceYModifier, BounceZModifier, EaseCurveModifier, EaseCurveXModifier, EaseCurveYModifier,
		EaseCurveZModifier, EaseCurveAngleModifier, InvertSineModifier, BoostModifier, BrakeModifier, JumpModifier
	];
	...
	override public function update(elapsed:Float)
	{
		...
		debugText.text = Std.string(FlxMath.roundDecimal(Conductor.songPosition / 1000, 2))
			...
			+ "\nBeat: "
			+ Std.string(curBeat).substring(0, 4)
			+ "\nStep: "
			+ curStep
			+ "\n";
	...
	private function generateSong(dataPath:String):Void
	{
		...
		unspawnNotes = ChartLoader.generateChartType(songData, "fnf");
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			var babyArrow:UIStaticArrow = ForeverAssets.generateUIArrows(0, 25 + (Init.trueSettings.get('Downscroll') ? FlxG.height - 200 : 0), i, PlayState.assetModifier);
			babyArrow.ID = i;
			babyArrow.x -= ((4 / 2) * Note.swagWidth);
	...
	function setupModifierUI()
	{
		...
		var modClassDropDown = new FlxUIDropDownMenu(modClassInputText.x, modClassInputText.y + 30,
			FlxUIDropDownMenu.makeStrIdLabelArray(modClassList, true), function(mod:String)
		{
		...
		var modTypeDropDown = new FlxUIDropDownMenu(modTypeInputText.x, modClassInputText.y + 30,
			FlxUIDropDownMenu.makeStrIdLabelArray(modTypeList, true), function(mod:String)
		{
	...
	function setupEventUI()
	{
		...
		easeDropDown = new FlxUIDropDownMenu(25, eventEaseInputText.y + 30, FlxUIDropDownMenu.makeStrIdLabelArray(easeList, true),
