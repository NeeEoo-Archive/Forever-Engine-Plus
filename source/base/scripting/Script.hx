...
class Script implements flixel.util.FlxDestroyUtil.IFlxDestroyable {
	 ...
	 */
	public static var file_extensions:Array<String> = ['script', 'templatescript'];
	...
	public static function load(path:String, call_new:Bool = true):Script {
		...
		for (ext in HScript.file_extensions) {
			...
			if (Assets.exists('${path}') && path.endsWith('.${ext}'))
				return new HScript('${path.substring(0, path.length - 1 - ext.length)}', call_new);
			// without ext
			if (Assets.exists('${path}.${ext}'))
				return new HScript('${path}.${ext}', call_new);
		}
	 ...
	 */
	public function get(item:String):Dynamic {
		return null;
	}
	 ...
	 */
	public function exists(item:String):Bool {
		return false;
	}
