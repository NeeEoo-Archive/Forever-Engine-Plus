...
import state.substate.OptionsSubstate;
...
class OptionsMenuState extends MusicBeatState
{
		...
		categoryMap = [
			...
			'preferences' => [
				[
					...
					['Accurate Fps', getFromOption],
					['Accurate Memory', getFromOption],
				]
			],
			'appearance' => [
				[
					['UI', null],
					['', null],
					["Show Timebar", getFromOption],
					["Score Bar Zoom On Hit", getFromOption],
					["Psych Icon Bops", getFromOption],
					...
					['', null],
					['Other', null],
					['Dark Window Border', getFromOption],
	...
	function selectOption(newSelection:Int, playSound:Bool = true)
	{
		...
		for (i in 0...activeSubgroup.length)
		{
			...
			activeSubgroup.members[i].xTo = 100;
			...
			if (categoryMap.get(curCategory)[0][i][1] == null)
			{
				...
				activeSubgroup.members[i].xTo = Std.int((FlxG.width - activeSubgroup.members[i].width) / 2);
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.BACK && !lockedMovement)
		{
			...
			if (Init.gameSettings.get(letter.text) != null)
			{
				switch (Init.gameSettings.get(letter.text)[1])
				{
					case Init.SettingTypes.Checkmark:
						// checkmark
						var checkmark = ForeverAssets.generateCheckmark(10, letter.y, 'checkboxThingie', 'base', 'UI');
